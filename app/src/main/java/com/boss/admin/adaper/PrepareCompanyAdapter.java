...
public class PrepareCompanyAdapter extends BaseAdapter {
    ...
    PrepareCompanyAdapter adap;
    List<Company> listdata;
    Company company;

    public PrepareCompanyAdapter(Context context, List<Company> hfListItems) {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        final PrepareCompanyAdapter.ViewHolder holder;
        if (convertView == null) {
            // 获得listview的item条目xml布局文件oww
            convertView = inflater.inflate(
                    R.layout.item_company, parent, false);
            holder = new PrepareCompanyAdapter.ViewHolder(convertView);
            // 最后setTag,也就是类似保存convertView，也就是保存了item
            convertView.setTag(holder);
        } else {
            // 如果convertView前面保存过，这里就不为空，那么我们把item拿过来用
            holder = (PrepareCompanyAdapter.ViewHolder) convertView.getTag();
        }
